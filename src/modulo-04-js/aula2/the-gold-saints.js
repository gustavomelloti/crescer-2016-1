...
function obterCavaleiroComMaisGolpes() {
    var cavaleiroMaiorNumGolpes = null;

    for (var i in goldSaints) {
        if (cavaleiroMaiorNumGolpes === null || goldSaints[i].golpes.length > cavaleiroMaiorNumGolpes.golpes.length)
            cavaleiroMaiorNumGolpes = goldSaints[i];
    }

    return cavaleiroMaiorNumGolpes;
}

function obterMesesComMaisAniversarios() {
    for (var i in goldSaints) {
        console.log(new Date(goldSaints[i].dataNascimento).getMonth());
    }
}
...
function obterAlturaMedia() {
    ...
    for (var i in goldSaints) {
        altura+= goldSaints[i].alturaCm;
    }

    return ((altura/goldSaints.length) / 100).toFixed(2);
}

function obterSobrepeso () {
    return goldSaints.filter(function(e){
        var imc = (e.pesoLb / 2.2046) / Math.pow(e.aluraCm /100, 2);
