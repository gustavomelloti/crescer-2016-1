...
function obterAlturaMedia() {
    return formatarNumero((Object.keys(goldSaints).reduce(function(previous, key) {
        return previous + goldSaints[key].alturaCm;
    }, 0) / goldSaints.length / 100));
}
...
function obterAlturaMediana() {
    ...
    return formatarNumero((alturas.length % 2 === 0 ? (alturas[posicao] + alturas[posicao]) / 2  : alturas[posicao]) / 100)
...
function obterPesoMedioCavaleiros(cavaleiros) {
    ...
    return formatarNumero((pesos.length % 2 === 0 ? (pesos[posicao] + pesos[posicao]) / 2  : pesos[posicao]) / 2.2046);
}

/* Exercício 7 */
function obterIMC() {
    return goldSaints.map(function(e){
        return calcularIMC(e.alturaCm/100, e.pesoLb / 2.2046);
    }).filter(Number);
}

function calcularIMC(altura, peso) {
    return formatarNumero((peso / Math.pow(altura, 2)));
}
