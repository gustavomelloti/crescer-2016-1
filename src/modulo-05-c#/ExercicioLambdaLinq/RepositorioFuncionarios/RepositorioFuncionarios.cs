...
namespace Repositorio
{
    public class RepositorioFuncionarios
    {
        ...
        {
            return Funcionarios.Where(x => x.Cargo.Equals(cargo)).ToList();
        }

        public IList<Funcionario> OrdenadosPorCargo()
        {
            return Funcionarios.OrderBy(x => x.Cargo.Titulo).ThenBy(x => x.Nome).ToList();
        }
        ...
        {
            return Funcionarios.Where(x => turnos.Contains(x.TurnoTrabalho)).ToList();
        }        
        ...
        public IList<Funcionario> FiltrarPorIdadeAproximada(int idade)
        {
            ...
            return Funcionarios.Where(
                x => anoAtual - x.DataNascimento.Year <= idade + 5 && anoAtual - x.DataNascimento.Year >= idade - 5 
            ).ToList();
        }        

        public double SalarioMedio(TurnoTrabalho? turno = null)
        {
            var funcionarios = turno == null ? Funcionarios : Funcionarios.Where(x => x.TurnoTrabalho == turno);
            return funcionarios.Average(x => x.Cargo.Salario);
        }

        public IList<Funcionario> AniversariantesDoMes()
        {
            return Funcionarios.Where(x => x.DataNascimento.Month == DateTime.Now.Month).ToList();
        }
        ...
        {
            return Funcionarios.GroupBy(
                x => x.TurnoTrabalho
            ).Select(x => new { Quantidade = x.Count(), Turno = x.Key }).ToArray();
        }
        ...
        public dynamic FuncionarioMaisComplexo()
        {
            return Funcionarios.Select(x => new
            {
                ...
                SalarioRS = String.Format ("R$ {0}", x.Cargo.Salario.ToString("F")),
                SalarioUS = String.Format("${0}", x.Cargo.Salario.ToString("F").Replace(',', '.')),
                QuantidadeMesmoCargo = Funcionarios.Where(y => y.Cargo.Equals(x.Cargo)).GroupBy(y => y.Cargo.Titulo).Select(a => a.Count()).ToArray()[0]
            }).Where(x => x.Nome.Equals("Dollynho Developer")).ToArray()[0];
        }
    }
}
